...
import { Stack, useRouter, useSegments, useNavigationContainerRef, useFocusEffect } from 'expo-router';
  ...
  useEffect(() => {
    ...
    const handleNavigation = async () => {
      try {
        ...
        if (isSignedIn) {
          // If user is not in a protected group, redirect based on their role
          if (!inProtectedGroup) {
            // TODO: Replace this with actual role check from user metadata
            const userRole = 'doctor'; // Default to doctor for now
            const targetPath = `/(${userRole})/dashboard`;
            
            // Only navigate if we're not already on the target path
            if (targetPath !== currentPath) {
              console.log(`[AuthProvider] Signed in, redirecting to: ${targetPath}`);
